...
Place the 9 target image files in a directory, then run the tool from that
directory (or build the binary and run it there):
...
- Exactly 9 matching images must be found, or the command fails with an
  error such as `found 5 image files, want exactly 9`.
- Supported image extensions: `.jpg`, `.jpeg`, `.png`.
- Matching files are sorted by filename for a stable, reproducible layout.
